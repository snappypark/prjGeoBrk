...
namespace nj
{
    public abstract class ObjsPartialPool<T, U> : ObjsPool<T, U> where T : MonoBehaviour where U : pqObj
    {
        ...
        public void Query_EnqueOrDequeByReturn(Pt centPt, Func<U, qReturn> callback)
        {
            ...
            {
                Pt qPt = centPt + idx.Pt3x3(qIdx);
                if (coord.IsOutOfIdx_OnPartial(qPt))
                    continue;
        ...
        {
            for (byte type = 0; type < NumType; ++type)
                for (int qi = 0; qi < coord.size.x; ++qi)
                    for (int qj = 0; qj < coord.size.y; ++qj)
                    {
        ...
        {
            U result = null;
            Query_EnqueOrDequeByReturn(pos.Pt(), delegate (U obj)
            {
