...
public class lineDraw : MonoBehaviour
{
    ...
    {
        _renderer.SetPositions(new Vector3[] { pos.WithGapY(_depth), pos.WithGapY(_depth) });
    }
    ...
    */
    ...
    public void InitLine_CommonOption( Vector3 p0, Vector3 p1, float width, Color color)
    {
        _renderer.DisableRendererOption();
        _renderer.widthMultiplier = width;//endWidth, startWidth = width;
        _renderer.numCornerVertices = 4;
        _renderer.numCapVertices = 4;
        _renderer.startColor = color;
        _renderer.endColor = color;
        _renderer.SetPositions(new Vector3[] { p0, p1 });
    }
